...
void Grid::reset()
{
	...
	std::fill( m_colpickers, m_colpickers + sz, 0);
}
...
int Grid::getColourPicker( int x, int y ) const
{
	return m_colpickers[ y * m_dim + x ];
}
...
void Grid::setColourPicker( int x, int y, int cp )
{
	m_colpickers[ y * m_dim + x ] = cp;
}
