...
const float SHAPE_SIZE_MAX = 1.0f;
const float SHAPE_SIZE_MIN = 0.0f;
const float SHAPE_RESIZE_AMOUNT = SHAPE_SIZE_MAX / 20;

// DEFAULTS
const float SHAPE_SIZE_DEFAULT = SHAPE_SIZE_MAX;
const float SHAPE_COLOR_DEFAULT = 1.0f;
const float SHAPE_ROTATION_DEFAULT = 0.0f;
const vec2 SHAPE_TRANSLATION_DEFAULT = vec2(0.0f);
...
// Constructor
A0::A0()
	: m_shape_color(glm::vec3(SHAPE_COLOR_DEFAULT, SHAPE_COLOR_DEFAULT, SHAPE_COLOR_DEFAULT)),
	  m_shape_translation(SHAPE_TRANSLATION_DEFAULT),
	  m_shape_size(SHAPE_SIZE_DEFAULT),
	  m_shape_rotation(SHAPE_ROTATION_DEFAULT),
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			if (m_shape_size <= SHAPE_SIZE_MAX - SHAPE_RESIZE_AMOUNT) {
				m_shape_size += SHAPE_RESIZE_AMOUNT;
			}
			else {
				m_shape_size = SHAPE_SIZE_MAX;
			}
		...
		if (key == GLFW_KEY_MINUS) {
			...
			if (m_shape_size >= SHAPE_SIZE_MIN + SHAPE_RESIZE_AMOUNT) {
				m_shape_size -= SHAPE_RESIZE_AMOUNT;
			}
			else {
				m_shape_size = SHAPE_SIZE_MIN;
			}
...
void A0::resetShape() {
	m_shape_size = SHAPE_SIZE_DEFAULT;
	m_shape_rotation = SHAPE_ROTATION_DEFAULT;
	m_shape_color.r = SHAPE_COLOR_DEFAULT;
	m_shape_color.g = SHAPE_COLOR_DEFAULT;
	m_shape_color.b = SHAPE_COLOR_DEFAULT;
	m_shape_translation = SHAPE_TRANSLATION_DEFAULT;
}